...
__memX DSPfract __attribute__((__aligned__(ECHO_MAX_LENGTH)))  echo_buffer[ECHO_MAX_LENGTH];
...
__memY DSPfract* readPtrOffset;
__memX const DSPfract* pInitGain;
__memY const DSPint* pInitDelay;
...
//
/////////////////////////////////////////////////////////////////////////////////
void multitap_echo(__memX DSPfract *pInbuf, __memX DSPfract *pOutbuf)
{
	/* TO DO: Add code from model 2 */
	DSPint i, j;
	__memX DSPfract* writePtr = echoState.writePointer;
	__memY DSPfract* pBufEnd = echoState.pEchoBuff+echoState.bufferLength - 1;
	...
	for(i = 0; i < BLOCK_SIZE; i++)
	{
		*writePtr = *pInbuf;
		...
		writePtr = CIRC_INC(echo_buffer, 0xb000); //0xb00 nm reg <=> module 4096
		...
		for(j = 0; j < echoState.n_tap; j++)
		{
			...
			readPtr = CIRC_INC(echo_buffer, 0xb000); //0xb00 nm reg <=> module 4096
			*readPtrPtr = readPtr;
			readPtrPtr++;
		}
